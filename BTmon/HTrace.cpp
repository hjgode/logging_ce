...
)
{
    ...
    if(p_pByteOffset)
    {
        ...
        l_dwOffsetLow = SetFilePointer(m_hFileHandle, 
            l_dwOffsetLow, &l_dwOffsetHigh, FILE_BEGIN);
    ...
    if(!(p_bWrite?
        WriteFile(m_hFileHandle, p_lpBuffer, 
        p_dwNumberOfBytes, &l_dwBytesProcessed, NULL):
        ReadFile(m_hFileHandle, p_lpBuffer, 
        p_dwNumberOfBytes, &l_dwBytesProcessed, NULL)))
    {
        l_dwError = GetLastError();
